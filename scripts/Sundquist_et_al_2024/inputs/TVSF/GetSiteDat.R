...
locs <- read.csv('scripts/inputs/TVSF/intermediate_data/sitedat_input.csv') %>%
  dplyr::select(site, name, latitude, longitude)

#1deg_grid for grabbing correct ABoVE DEM tiles
latlon_grid <- readOGR(dsn = '/scratch/af2358/UVAFME_Parameterization/GIS_Data',
...
write.csv(siteDat, "scripts/inputs/TVSF/intermediate_data/soildat_output.csv", quote = F, row.names = F)
