...
initclims = c("","/r45","/r85")
iscen = c("init","spinup")
# lclims = c("0degCC","1degCC","2degCC","4degCC")
lclims = c("0degCC")
forests = c("bsp","wsps","decid")
cmgmts = c("cf","harv","thin30","shear","shearplant","prune","harvplant","thin60")

mgmt = fread("input_data/mgmt_testing/main/UVAFME2018_active_management.csv")
sites = fread("input_data/mgmt_testing/main/UVAFME2018_site.csv")
...
cols[["Fuel_Conds.csv"]] = c("siteID","plot","year","day","fuel_sum","fuel_dec","fuel_con","fuel_twig","fuel_smbr",
"fuel_lgbr","fuel_bole","fuel_moss","fuel_dmoss","fuel_root","fuel_shrub","BD_dec","BD_con","BD_twig","BD_smbr","BD_lgbr",
"BD_bole","BD_moss","BD_dmoss","BD_root","BD_shrub","SAV_dec","SAV_con","SAV_twig","SAV_smbr","SAV_lgbr","SAV_bole",
"SAV_moss","SAV_dmoss","SAV_root","SAV_shrub","moist_dec","moist_con","moist_twig","moist_smbr","moist_lgbr","moist_bole",
"moist_moss","moist_dmoss","moist_root","moist_shrub","mef_dec","mef_con","mef_twig","mef_smbr","mef_lgbr","mef_bole",
"mef_moss","mef_dmoss","mef_root","mef_shrub","sumlit_moist","sumlit_SAV","sumlit_BD","MEF")

cols[["Fire_Conds.csv"]] = c("siteID","plot","year","day","FDI","ffmc","MEF","fuel_moisture","fuel_BD","fuel_SAV","fuel_dec",
"fuel_con","fuel_twigs","fuel_smbr","fuel_lbr","fuel_bole","fuel_mosslive","fuel_moss","fuel_root","fuel_shrub","Uf","A",
"beta","beta_op","beta_frac","rVol_max","rVol_opt","n_M","net_fuel","I_r","flux_rat","phi_wind","Qig","rosf","a_f","burn_dec",
"burn_con","burn_twig","burn_smbr","burn_lgbr","burn_bole","burn_moss","burn_dmoss","burn_root","burn_shrub","I_surf","tau_l")
...
mgmtslong = c("Counterfactual","Harvest","Harvest + replanting","Thinning","Thinning","Shearblading","Shearblading + replanting","Pruning")
mgmthash = hash(mgmts, mgmtslong)
# climpal
gcmpal =  c(brewer.pal(11, "RdBu")[c(7,4,2)])
gcmpal2 =  c(brewer.pal(11, "RdBu")[c(8,3,1)])
...
transparent <- theme(
  ...
  biglabels <- theme(text = element_text(size=20))
  rotate_x_text <- theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
  remove_legend <- guides(fill="none", color = "none")
...
mgmtnames <- function(x) {
  x[x == "cf"] = "No management"
  x[x == "bau"] = "Business as usual"
  x[x == "prj"] = "Projected"
  return(x)
}

treatmentnames <- function(x) {
  x[x == "cf"] = "Nothing"
  x[x == "harv"] = "Harvest"
  x[x == "thin30"] = "Thin"
  x[x == "thin60"] = "Thin"
  x[x == "shear"] = "Shearblade"
  x[x == "prune"] = "Prune"
  return(x)
}

assignforest <- function(x) {
  wsps = fread(paste0(inDir, siteDir, "wsps/UVAFME2018_sitelist.csv"))
  bsp = fread(paste0(inDir, siteDir, "bsp/UVAFME2018_sitelist.csv"))
  decid = fread(paste0(inDir, siteDir, "decid/UVAFME2018_sitelist.csv"))
  
  x$forest = ""
  x[x$site %in% wsps$siteID,]$forest = "wsps"
  x[x$site %in% bsp$siteID,]$forest = "bsp"
  x[x$site %in% decid$siteID,]$forest = "decid"
