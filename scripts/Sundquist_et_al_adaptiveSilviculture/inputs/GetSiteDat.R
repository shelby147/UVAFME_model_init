...
libpaths <- c('/scratch/af2358/R_libs',
              '/packages/R/4.0.2-pngjpeg/lib64/R/library')
.libPaths(libpaths)
...
get_siteDat <- function(locs) {
  ...
  for (i in 1:length(latlons)){
    ...
    dem <- raster(paste0('/DEM_mosaics/v2_1/all_ABoVE/2_DEMs/',
                         'ABoVE_composDEM_0p33AS_', latlons[i], '.tif'))

    slope <- raster(paste0('/v2p1/slope_deg/ABoVE_composDEM_0p33AS_',
                           latlons[i], '_slope_deg.tif'))

    aspect <- raster(paste0('/v2p1/aspect_deg/ABoVE_composDEM_0p33AS_',
  ...
  ## Ecoregions
  eco <- readOGR(dsn = '/scratch/ss3988/polygons/', layer = 'ABOVE_Ecoregions_L3')
  ## Extract all values
...
write.csv(siteDat, "/home/ss3988/UVAFME/TVSF_results/scripts/landscape_inputs/scripts/intermediate_data/Landsat_init_soildat_output.csv", quote = F, row.names = F)
