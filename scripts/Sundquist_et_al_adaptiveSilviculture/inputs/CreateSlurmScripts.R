...
model = "UVAFME_init"
jobdir = "jobfiles/mgmt_testing/"
flistdir = c("file_lists/mgmt_testing/")
...
for(m in mscen[1]) {
  for(c in cscen) {
    for(u in nmunits) {
      ...
      mem = 2000 #this is running without management; no big reqs
      
      l2 = paste0("#SBATCH -o logs/mgmt_testing/",name,"_out.log")
      l3 = paste0("#SBATCH -o logs/mgmt_testing/",name,"_err.log")
      l6 = paste0("#SBATCH --mem=",mem)
      l7 = paste0("#SBATCH --time=",time)
      l8 = paste0("name=file_lists/mgmt_testing/",m,"/",c,"/file_list_",u,".txt")
...
for(m in mscen[2:3]) {
  for(c in cscen) {
    for(u in munits) {
      ...
      td = seconds_to_period(time)
      time = sprintf('%02d:%02d:%02d', td@hour, minute(td), second(td))
      mem = mempersite*nunits[nunits$unit == u,]$n
...
# Not sure why, but the cf_7C sites are all running out of memory. Raising these to 4000
