...
contains
  ...
  !:...........................................................................:
    ...
    subroutine open_inputFiles
        ...
        ! Litter parameters
        call open_and_check('_litterpars.csv', splistdir, litterfile, 1,         &
            'litter')
        ...
        ! Monthly climate file
        if (use_monthly_clim) then
            call open_and_check('_climate_monthly.csv', inputdir, mclimfile,   &
                1, 'monthlyclim')
        end if
    ...
    subroutine open_outputFiles
        ...
        call build_pathname(outputdir, 'Forestry_Data.csv', pathname)
        forestry = open_file(pathname)


        call build_pathname(outputdir, 'Active_Management_Data.csv', pathname)
        mgmt = open_file(pathname)
        ...
        if (mgmt_testing) then 
            call build_pathname(outputdir, 'Fire_Summary.csv', pathname)
            sitefire = open_file(pathname)

            call build_pathname(outputdir, 'Regen_Summary.csv', pathname)
            siteregen = open_file(pathname)
        end if 
    ...
    subroutine write_headers()
        ...
        call csv_write(soildecomp, 'n_used', .false.)
        ...
        call csv_write(clim_unit, 'summerVPD', .false.)
        ...
        call csv_write(biom_by_g, 'qmean_diam', .false.)
        ...
        ! call csv_write(biom_by_s, '5 to 7', .false.)
        ! call csv_write(biom_by_s, '7 to 10', .false.)
        ! call csv_write(biom_by_s, '10 to 15', .false.)
        ! call csv_write(biom_by_s, '15 +', .false.)
        ! call csv_write(biom_by_s, '0 to 5 biom', .false.)
        ! call csv_write(biom_by_s, '5 to 7 biom', .false.)
        ! call csv_write(biom_by_s, '7 to 10 biom', .false.)
        ! call csv_write(biom_by_s, '10 to 15 biom', .false.)
        ! call csv_write(biom_by_s, '15 + biom', .false.)
        call csv_write(biom_by_s, '0 to 5', .false.)
        ...
        call csv_write(biom_by_s, 'qmean_diam', .false.)
        call csv_write(biom_by_s, 'mean_age', .false.)
        call csv_write(biom_by_s, 'max_hgt', .false.)
        call csv_write(biom_by_s, 'recr_trees', .false.)
        ...
        call csv_write(dead_s, '0 to 5', .false.)
        call csv_write(dead_s, '5 to 10', .false.)
        call csv_write(dead_s, '10 to 20', .false.)
        call csv_write(dead_s, '20 to 30', .false.)
        call csv_write(dead_s, '30 to 40', .false.)
        call csv_write(dead_s, '40 to 50', .false.)
        call csv_write(dead_s, '50 to 60', .false.)
        call csv_write(dead_s, '60 to 70', .false.)
        call csv_write(dead_s, '70 to 80', .false.)
        call csv_write(dead_s, '80 to 90', .false.)
        call csv_write(dead_s, 'over 90', .false.)
        call csv_write(dead_s, '0 to 5 biom', .false.)
        call csv_write(dead_s, '5 to 10 biom', .false.)
        call csv_write(dead_s, '10 to 20 biom', .false.)
        call csv_write(dead_s, '20 to 30 biom', .false.)
        call csv_write(dead_s, '30 to 40 biom', .false.)
        call csv_write(dead_s, '40 to 50 biom', .false.)
        call csv_write(dead_s, '50 to 60 biom', .false.)
        call csv_write(dead_s, '60 to 70 biom', .false.)
        call csv_write(dead_s, '70 to 80 biom', .false.)
        call csv_write(dead_s, '80 to 90 biom', .false.)
        call csv_write(dead_s, 'over 90 biom', .false.)
        ...
        call csv_write(plotvals, 'LAI_12', .false.)
        call csv_write(plotvals, 'init_strength', .true.)

        !Forestry data
        call csv_write(forestry,'siteID', .false.)
        call csv_write(forestry, 'runID', .false.)
        call csv_write(forestry, 'year', .false.)
        call csv_write(forestry, 'species', .false.) !one for each species plus one for aggregate
        call csv_write(forestry, 'reproduction', .false.) !count of sized stems
        call csv_write(forestry, 'poletimber', .false.)
        call csv_write(forestry, 'sawtimber', .false.)
        call csv_write(forestry, 'reproduction_sd', .false.) !between plots
        call csv_write(forestry, 'poletimber_sd', .false.)
        call csv_write(forestry, 'sawtimber_sd', .false.)
        ! call csv_write(forestry, 'max_height', .false.) !can get these more or less in species_data and plot_data
        ! call csv_write(forestry, 'max_height_sd', .false.)
        ! call csv_write(forestry, 'Loreys_height', .false.)
        ! call csv_write(forestry, 'Loreys_height_sd', .true.)
        call csv_write(forestry, 'cf_mn', .false.)
        call csv_write(forestry, 'cf_sd', .false.)
        call csv_write(forestry, 'bf_mn', .false.)
        call csv_write(forestry, 'bf_sd', .false.)
        call csv_write(forestry, 'LAI_0_2', .false.)
        call csv_write(forestry, 'LAI_2_4', .false.)
        call csv_write(forestry, 'LAI_4_6', .false.)
        call csv_write(forestry, 'LAI_6_8', .false.)
        call csv_write(forestry, 'LAI_8_10', .false.)
        call csv_write(forestry, 'LAI_10_12', .false.)
        call csv_write(forestry, 'LAI_12_14', .false.)
        call csv_write(forestry, 'LAI_14_16', .false.)
        call csv_write(forestry, 'LAI_16_18', .false.)
        call csv_write(forestry, 'LAI_18_20', .false.)
        call csv_write(forestry, 'LAI_20_22', .false.)
        call csv_write(forestry, 'LAI_22_24', .false.)
        call csv_write(forestry, 'LAI_24_26', .false.)
        call csv_write(forestry, 'LAI_26_28', .false.)
        call csv_write(forestry, 'LAI_28_30', .false.)
        call csv_write(forestry, 'LAI_30_32', .false.)
        call csv_write(forestry, 'LAI_32_34', .false.)
        call csv_write(forestry, 'LAI_34_36', .false.)
        call csv_write(forestry, 'LAI_36_38', .false.)
        call csv_write(forestry, 'LAI_38_40', .false.)
        call csv_write(forestry, 'LAI_40_60', .true.)


        ! Management data
        call csv_write(mgmt, 'siteID', .false.)
        call csv_write(mgmt, 'year', .false.)
        call csv_write(mgmt, 'classification', .false.)
        call csv_write(mgmt, 'mgmtcode', .true.)
        ...
        if (plot_level_data) then
           ...
           call csv_write(pl_biom_by_g, 'qmean_diam', .false.)
           ...
           call csv_write(pl_biom_by_s, '5 to 7', .false.)
           call csv_write(pl_biom_by_s, '7 to 10', .false.)
           call csv_write(pl_biom_by_s, '10 to 15', .false.)
           call csv_write(pl_biom_by_s, '15 +', .false.)
           call csv_write(pl_biom_by_s, '0 to 5 biom', .false.)
           call csv_write(pl_biom_by_s, '5 to 7 biom', .false.)
           call csv_write(pl_biom_by_s, '7 to 10 biom', .false.)
           call csv_write(pl_biom_by_s, '10 to 15 biom', .false.)
           call csv_write(pl_biom_by_s, '15 + biom', .false.)
           ! call csv_write(pl_biom_by_s, '0 to 5', .false.)
           ...
           call csv_write(pl_biom_by_s, 'qmean_diam', .false.)
           ...
           ! call csv_write(dead_ps, '0 to 5', .false.)
           ! call csv_write(dead_ps, '5 to 7', .false.)
           ! call csv_write(dead_ps, '7 to 10', .false.)
           ! call csv_write(dead_ps, '10 to 15', .false.)
           ! call csv_write(dead_ps, '15 +', .false.)
           ! call csv_write(dead_ps, '0 to 5 biom', .false.)
           ! call csv_write(dead_ps, '5 to 7 biom', .false.)
           ! call csv_write(dead_ps, '7 to 10 biom', .false.)
           ! call csv_write(dead_ps, '10 to 15 biom', .false.)
           ! call csv_write(dead_ps, '15 + biom', .false.)
...
!            call csv_write(pl_tree, 'col', .false.)
            call csv_write(pl_tree, 'id', .false.)
            call csv_write(pl_tree, 'age', .false.)
            call csv_write(pl_tree, 'planted', .false.)
            call csv_write(pl_tree, 'diam', .false.)
!            call csv_write(pl_tree, 'dcbb', .false.)
            call csv_write(pl_tree, 'height', .true.)
...
!            call csv_write(pl_tree, 'nutrient_resp', .true.)
        ...
        if (testing) then
            ...
            call csv_write(dayfire, 'fuel_twigs', .false.)
            call csv_write(dayfire, 'fuel_smbr', .false.)
            call csv_write(dayfire, 'fuel_lbr', .false.)
            call csv_write(dayfire, 'fuel_bole', .false.)
            call csv_write(dayfire, 'fuel_mosslive', .false.)
        ...
        if (mgmt_testing) then 
            call csv_write(sitefire, 'siteID', .false.)
            call csv_write(sitefire, 'year', .false.)
            call csv_write(sitefire, 'max_FDI', .false.)
            call csv_write(sitefire, 'cum_FDI', .false.)
            call csv_write(sitefire, 'I_r', .false.)
            call csv_write(sitefire, 'I_r_sd', .false.)
            call csv_write(sitefire, 'NFires', .true.)

            call csv_write(siteregen, 'siteID', .false.)
            call csv_write(siteregen, 'year', .false.)
            call csv_write(siteregen, 'species', .false.)
            call csv_write(siteregen, 'regrowth', .false.)
            call csv_write(siteregen, 'regrowth_sd', .true.)
    ...
    subroutine close_outputFiles
        ...
        inquire(forestry, opened = isopen)
        if (isopen) close(forestry)

        inquire(mgmt, opened = isopen)
        if (isopen) close(mgmt)
