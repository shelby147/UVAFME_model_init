...
contains
    ...
    subroutine initialize_plot(self, numspecies, a_sat, a_fc, a_pwp, o_sat,   &
            o_fc, o_pwp, o_bd, a_bd, itxt, hum_input, A_depth, PCTN)
        !
        ...
        ! turn this into input file value/ hard code
        ...
        if(PCTN == 0.006) then !initialized black spruce
         self%soil%moss_biom = 120.0
        end if
    ...
    subroutine tree_dm_cats(self, genera, field, diam_categories, volume)
        !
        ...
        real,    dimension(size(genera) + 1, 2),     intent(out), optional :: volume


        ! Data dictionary: local variables
        character(len = MAX_NLEN) :: comp
        integer, dimension(NHC)   :: tree_diams
        real,    dimension(2)     :: tree_volume
        ...
        if(field == 'forestry') then !Different DBH binning
          volume = 0.0
          do ig = 1, numitems
              do n = 1, self%numtrees
                comp = self%trees(n)%spec_ptr%unique_id
                if (comp == genera(ig)) then
                    call timber_get_diam_category(self%trees(n), tree_diams, tree_volume, comp)
                    diam_categories(ig,:) = diam_categories(ig,:) + tree_diams
                    volume(ig,:) = volume(ig,:) + tree_volume
                endif
              enddo
            enddo
        else
    ...
    subroutine deadtree_dm_cats(self, genera, field, diam_categories)
        !
        !  Bins the number of trees on a plot into DBH size classes, for
        !    writing to output files
        !
        ...
        ! Loop through genera and trees
        do ig = 1, numitems
            do n = 1, self%num_dead

                ! Get correct identifier - genus or species ID
                if (field == 'genus') then
                    comp = self%deadtrees(n)%spec_ptr%genus_name
                else if (field == 'species') then
                    comp = self%deadtrees(n)%spec_ptr%unique_id
                endif

                ! If we are on the right one - get the DBH categories
                if (comp == genera(ig)) then
                    call get_diam_category(self%deadtrees(n), tree_diams)
                    diam_categories(ig,:) = diam_categories(ig,:) + tree_diams
                endif
            enddo
        enddo

    end subroutine deadtree_dm_cats

    !:.........................................................................:

    subroutine sum_over_sg(self, genera, field, basal_area, leaf_bm, biomC,    &
        biomN, max_ht, max_diam, mean_dbh, qmean_dbh, mean_year, biom_cats, envresp_mn,   &
        ...
        real, dimension(size(genera)),           intent(out) :: qmean_dbh  ! Quadratic weighted mean DBH (cm)
        ...
        real, dimension(size(genera))         :: sum_qdbh      ! Sum of quadratic weighted diameters (cm)
        ...
        do is = 1, numitems
            do it = 1, self%numtrees
                ...
                if (comp == genera(is)) then
                    ...
                    sum_qdbh(is) = sum_qdbh(is) + self%trees(it)%diam_bht**2
