...
contains
    ...
    subroutine total_plot_values(site, year)
        ...
        real, dimension(FC_NUM)                :: envresp_4_sd ! Sd of growth response for DBH bin 4 (0-1)
        ...
        real :: init_basal_area ! Total basal area of all initialized trees
        real :: init_strength ! Percent basal area derived from initialized trees
        ...
        do i = 0, LAI_BINS - 1
          site_lai(i+1) = site%lai_array(i*laibin_size + 1)
        end do


        ! Was causing quintuple counting
        ! do i = 0, LAI_BINS-1
        ...
        do ip = 1, site%numplots
            ...
            do it = 1, site%plots(ip)%numtrees
                ...
                ! Sum up basal area of initialized trees
                if(init_on) then 
                    if(site%plots(ip)%trees(it)%planted == 2) then 
                        init_basal_area = init_basal_area + basal_area(ip, it)
                    end if 
                end if 
        ...
        ! Calculate percent BA derived from initialized trees 
        init_strength = init_basal_area/sum(plbasal_area(1:numplots))
        ...
        call csv_write(plotvals, site_lai(LAI_BINS), .false.)
        call csv_write(plotvals, init_strength, .true.)
  ...
  !:...........................................................................:
    ...
    subroutine write_genus_or_species_data(site, species_pres, year, field,    &
        ...
        real,    dimension(site%numplots, num_types)         :: qmean_diam    ! Quadratic average diameter (cm2)
        ...
        real,    dimension(num_types)                        :: qmean_diam_mn
        real,    dimension(num_types)                        :: qmean_diam_sd
        ...
        integer                                              :: ntrees        !Counter for allocating
        integer                                              :: ip, is, ns, l, ix, it ! Looping indices
        ...
        do ip = 1, site%numplots
            call sum_over_sg(site%plots(ip), genera, field, basal_area(ip, :), &
                ...
                max_diam(ip, :), mean_diam(ip, :), qmean_diam(ip,:), mean_age(ip, :),            &
        ...
        do is = 1, num_types
            ...
            call stddev(mean_diam(:, is), mean_diam_mn(is), mean_diam_sd(is),  &
                RNVALID)
            call stddev(qmean_diam(:, is), qmean_diam_mn(is), qmean_diam_sd(is),  &
                            RNVALID)
        ...
        do is = 1, num_types
            ...
            if (in_site(is)) then
                ...
                call csv_write(funit, qmean_diam_mn(is), .false.)
                call csv_write(funit, age_mn(is), .false.)
                call csv_write(funit, max_ht_mn(is), .false.)
                if (field == 'species' .and. present(species_ids) .and.        &
                    present(sndx)) then
                    locs = findloc(species_ids(sndx,:), value = is)
                    if(locs(1) > epsilon(1.0)) then 
                    call csv_write(biom_by_s, site%recr_trees(locs(1)), .false.)
                    else 
                        call csv_write(biom_by_s, -999, .false.)
                    end if 
                end if
                ...
                if (field == 'species' .and. present(species_ids) .and.        &
                    present(sndx)) then
                    numlines = NHC + NHC + FC_NUM + 26
                else
                    numlines = NHC + NHC + FC_NUM + 25
                end if
                do l = 1, numlines
            ...
            do ip = 1, site%numplots
                do is = 1, num_types
                    ...
                    if (in_site(is)) then
                        ...
                        call csv_write(funit_p, qmean_diam(ip, is), .false.)
    ...
    subroutine write_forestry_data(site, species_pres, year, funit)
      ! Writes output metrics to compare to forest inventory data
      ! by Shelby Sundquist

      ! Data dictionary
      ! Constants
      integer, parameter :: LA_BINS = 20 ! Number of la bins
      integer, parameter :: maxht = 40    ! Maximum height for la bins (m)
      ...
      !Local variables
      character(len=MAX_NLEN), dimension(size(site%species))    :: genera        ! Species/genus names
      character(len=MAX_NLEN), dimension(site%num_trees)        :: tree_spec        ! Species/genus names
      integer, dimension(site%numplots, site%num_trees, NHC)    :: diam_cats     ! Size structure (stems/dbh bin)
      integer, dimension(site%numplots, NSC)                    :: all_diam_cats ! Size structure (stems/dbh bin)
      real,    dimension(site%num_trees+1, NSC)                 :: diam_cats_mn ! Average size structure (stems/ha)
      real,    dimension(site%num_trees+1, NSC)                 :: diam_cats_sd ! Average size structure (stems/ha)
      real,    dimension(site%numplots, site%num_trees+1, 2)    :: volume ! Cubic feet (1) and board feet (2)
      real,    dimension(site%num_trees+1, LA_BINS+1)           :: site_la ! Per-species vertical-binned la (m2/m2)
      real,    dimension(site%num_trees+1)                      :: volume_cf_mn ! Average size structure (stems/ha)
      real,    dimension(site%num_trees+1)                      :: volume_cf_sd ! Average size structure (stems/ha)
      real,    dimension(site%num_trees+1)                      :: volume_bf_mn ! Per ha
      real,    dimension(site%num_trees+1)                      :: volume_bf_sd ! Average size structure (stems/ha)
      logical, dimension(size(site%species))                    :: in_site       ! Is the species/genus in the site?
      integer                                              :: labin_size  ! Size of la bins (m)
      integer                                              :: start, fin, numlines    !Markers
      integer                                              :: ip, ix, is, l, ns, i  ! Looping indices and end markers
      ...
      do is = 1, size(site%species)
          do ns = 1, size(site%species)
              if (genera(is) == site%species(ns)%unique_id .and. site%species(ns)%form == 1) then
                  in_site(is) = .true.
                  i = i + 1
                  tree_spec(i) = site%species(ns)%unique_id
                  exit
              endif
        enddo
      enddo
      ...
      ! Size class and volume binning
      do ip = 1, site%numplots
        call tree_dm_cats(site%plots(ip), tree_spec, 'forestry',                  &
          diam_cats(ip, :, :), volume(ip,:,:))
        end do

      do is = 1, site%num_trees
          do ix = 1, NSC
            call stddev(float(diam_cats(:, is, ix)), diam_cats_mn(is, ix),          &
            diam_cats_sd(is, ix), RNVALID)

            if (in_site(is)) then
              do ip = 1, site%numplots
                all_diam_cats(ip, ix) = all_diam_cats(ip, ix) +              &
                    float(diam_cats(ip, is, ix))
              enddo
            endif
          end do

          call stddev(volume(:,is,1), volume_cf_mn(is), volume_cf_sd(is), RNVALID)
          call stddev(volume(:,is,2), volume_bf_mn(is), volume_bf_sd(is), RNVALID)
        enddo

        do i = 1, site%num_trees
          do ip = 1, site%numplots
            volume(ip,site%num_trees+1,:) = volume(ip,site%num_trees+1,:) + &
            volume(ip,i,:)
          end do
        end do

      !  volume(ip, site%num_trees+1,:) = sum(volume(ip, :, :))

        call stddev(volume(:,site%num_trees+1,1), volume_cf_mn(site%num_trees+1), &
        volume_cf_sd(site%num_trees+1), RNVALID)
        call stddev(volume(:,site%num_trees+1,2), volume_bf_mn(site%num_trees+1), &
        volume_bf_sd(site%num_trees+1), RNVALID)

        do ix = 1, NSC
          call stddev(float(all_diam_cats(:,ix)), diam_cats_mn(site%num_trees+1, ix),       &
            diam_cats_sd(site%num_trees+1, ix), RNVALID)
        enddo

        !trees/ha
        diam_cats_mn = diam_cats_mn*HEC_TO_M2/plotsize
        diam_cats_sd = diam_cats_sd*HEC_TO_M2/plotsize
        volume_cf_mn = volume_cf_mn*HEC_TO_M2/plotsize
        volume_cf_sd = volume_cf_sd*HEC_TO_M2/plotsize
        volume_bf_mn = volume_bf_mn*HEC_TO_M2/plotsize
        volume_bf_sd = volume_bf_sd*HEC_TO_M2/plotsize

        ! la binning
        labin_size = maxht/(LA_BINS) !2m each
        do i = 0, LA_BINS-1
            start = i*labin_size + 1
            fin = i*labin_size + labin_size
            do is = 1, site%num_trees
             site_la(is,i + 1) = sum(site%la_spec(is,start:fin))
            end do
           site_la(site%num_trees + 1,i + 1) = sum(site%la_spec(1:site%num_trees, start:fin))
        end do
        do is = 1, site%num_trees

         site_la(is,LA_BINS + 1) = sum(site%la_spec(is,fin:maxheight))
        end do

       site_la(site%num_trees + 1,LA_BINS + 1) = sum(site%la_spec(1:site%num_trees,fin:maxheight))

        do is = 1, site%num_trees
            call csv_write(funit, site%site_id, .false.)
            call csv_write(funit, site%runID, .false.)
            call csv_write(funit, year, .false.)
            call csv_write(funit, tree_spec(is), .false.)

              do l = 1, NSC
                call csv_write(funit, diam_cats_mn(is, l), .false.)
                  call csv_write(funit, diam_cats_sd(is, l), .false.)
              end do
              call csv_write(funit, volume_cf_mn(is), .false.)
              call csv_write(funit, volume_cf_sd(is), .false.)
              call csv_write(funit, volume_bf_mn(is), .false.)
              call csv_write(funit, volume_bf_sd(is), .false.)
              do i = 1, LA_BINS
                  call csv_write(funit, site_la(is, i), .false.)
              end do
              call csv_write(funit, site_la(is, LA_BINS+1), .true.)
          !   else
          !     !write -999s
          !     numlines = NSC*2 + LA_BINS+1 + 4
          !     do l = 1, numlines - 1
          !       call csv_write(funit, RNVALID, .false.)
          !     enddo
          !     call csv_write(funit, RNVALID, .true.)
        ...
        !Write line for all  species combined
        call csv_write(funit, site%site_id, .false.)
        call csv_write(funit, site%runID, .false.)
        call csv_write(funit, year, .false.)
        call csv_write(funit, 'ALL', .false.)
        do l = 1, NSC
          call csv_write(funit, diam_cats_mn(site%num_trees + 1, l), .false.)
          call csv_write(funit, diam_cats_sd(site%num_trees + 1, l), .false.)
        end do
          call csv_write(funit, volume_cf_mn(site%num_trees+1), .false.)
          call csv_write(funit, volume_cf_sd(site%num_trees+1), .false.)
          call csv_write(funit, volume_bf_mn(site%num_trees+1), .false.)
          call csv_write(funit, volume_bf_sd(site%num_trees+1), .false.)
          do i = 1, LA_BINS
              call csv_write(funit, site_la(site%num_trees + 1, i), .false.)
          end do
          call csv_write(funit, site_la(site%num_trees + 1, LA_BINS+1), .true.)

    end subroutine write_forestry_data

    !:.........................................................................:

    subroutine write_management_data(site, year, funit)
      ...
      call csv_write(funit, site%site_id, .false.)
      call csv_write(funit, year, .false.)
      call csv_write(funit, site%classification, .false.)
      call csv_write(funit, site%mgmtflag, .true.)

    end subroutine write_management_data
    ...
    subroutine write_dead_genus_or_species_data(site, species_pres, year,      &
        ...
        real,    dimension(num_types)                         :: mean_diam_mn  ! Average diameter (cm)
        real,    dimension(num_types)                         :: mean_diam_sd  ! SD of diameter (cm)
        integer, dimension(site%numplots, num_types, NHC)    :: diam_cats_dead     ! Size structure (stems/dbh bin)
        real,    dimension(site%numplots, num_types, NHC)    :: biom_cats_dead     ! Biomass by dbh bin (tC)
        real,    dimension(num_types, NHC)                    :: tot_diam_cats_dead
        real,    dimension(num_types, NHC)                    :: tot_biom_cats_dead
        integer, dimension(num_types)                         :: spec_index    ! Location of present species/genera
        logical, dimension(num_types)                         :: in_site       ! Is the species/genus in the site?
        character(len=MAX_NLEN)                               :: comp          ! Species/genus name
        character(len=MAX_NLEN)                               :: genname       ! Genus name
        integer                                               :: ntrees
        integer                                               :: ip, is, ns, ix, it, l ! Looping indices
        ...
        do ip = 1, site%numplots
            call sum_over_deadsg(site%plots(ip), genera, field, biomC(ip, :),      &
                  mean_diam(ip, :), d_markers(ip, :, :), biom_cats_dead(ip,:,:))

            call deadtree_dm_cats(site%plots(ip), genera, field,                   &
                diam_cats_dead(ip, :, :))
        enddo

        tot_diam_cats_dead = 0.0
        tot_biom_cats_dead = 0.0

        ! Accumulate across-plot data
        do is = 1, num_types
            do ip = 1, site%numplots
                if (in_site(is)) then
                    tot_diam_cats_dead(is, :) = tot_diam_cats_dead(is, :) +              &
                        float(diam_cats_dead(ip, is, :))
                    tot_biom_cats_dead(is, :) = tot_biom_cats_dead(is, :) +              &
                        biom_cats_dead(ip, is, :)
                endif
            enddo
          end do

        !trees/ha and tC/ha
        tot_diam_cats_dead = tot_diam_cats_dead*HEC_TO_M2/plotsize/float(site%numplots)
        tot_biom_cats_dead = tot_biom_cats_dead*HEC_TO_M2/plotsize/float(site%numplots)
...
!!!!!!!!!
        ...
        do ip = 1, site%numplots
            call sum_over_deadsg(site%plots(ip), genera, field, biomC(ip, :),  &
                mean_diam(ip, :), d_markers(ip, :, :), biom_cats_dead)
        enddo
    ...
    subroutine write_soiln_data(site, year)
        ...
        real, dimension(site%numplots)           :: n_used       ! Plant-available N (kgN/ha)
        ...
        do ip = 1, site%numplots
            ...
            n_used(ip) = site%plots(ip)%soil%N_used*T_TO_KG
        ...
        call stddev(n_used, n_used_mn, n_used_sd, RNVALID)
        ...
        call csv_write(soildecomp, n_used_mn, .false.)
    ...
    subroutine write_tree_data(site, year)
        ...
        do ip = 1, site%numplots
            do it = 1, site%plots(ip)%numtrees
              if(year>0) then
              !if(site%plots(ip)%trees(it)%tree_age > 30) then
              !if(site%plots(ip)%trees(it)%spec_ptr%host == 1) then
