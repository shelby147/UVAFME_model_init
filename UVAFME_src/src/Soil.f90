...
!*******************************************************************************
    ...
    real,    parameter :: MOSS_SAV = 200.0  ! Moss SAV (/cm)
    ...
    type SoilData
        ...
        real, dimension(FL_LEVS)            :: litSAV        ! Fuel Surface-area to volume ratio (/cm)
  ...
  contains
    ...
    subroutine moist(soil, siteID, ip, year, day, ta, rain, pot_ev_day, lai ,  &
        ...
        if (use_monthly_clim .and. present(soilW) .and. present(soilI)) then
            H2Oice(2) = soilI
            water(2) = soilW
            wc(2) = ((water(2) + H2Oice(2))/zh(2)*H2O_D/ABD)
        end if
    ...
    subroutine moss(soil, alff, cla, decLit, drydays, siteID, plot, year)
        ...
        real, parameter :: SPORES = 0.001 ! Proportion of biomass spent on reproduction
        real, parameter :: PMAX = 0.35    ! Maximum moss production (kg/m2)
            ...
            do i = 2, nc
                ...
                if (lt /= IMOSS) then
                    ...
                    if (perc_rem <= C(i, 12)) then
                        ...
                        ! Transfer cohorts to humus or well-decayed wood
                        if (C(i, 6) == 1.0) then
                            ! Humus
    ...
    subroutine bulldoze_soil(soil, prem)
        !
        !  Calculates removal of litter and humus layers from shearblading
        !
        !  Record of revisions:
        !      Date       Programmer          Description of change
        !      ====       ==========          =====================
        ...
        ! Data dictionary: calling arguments
        class(SoilData),          intent(inout) :: soil ! Soil object
        real,                     intent(in)    :: prem ! Percent to remove via bulldozing

        ! Data dictionary: local variables
        real, dimension(NCOH_MAX, NCOH_CHR)  :: C         ! Local array of decaying cohorts
        real                                 :: duff_loss ! Loss from humus layer (t/ha)
        real                                 :: Nloss     ! Nloss from humus layer (tN/ha)
        real                                 :: loss      ! Loss from a cohort (t/ha)
        real                                 :: m_loss    ! Loss from moss biomass (kg)
        real                                 :: con_fuel  ! Conifer litter (kg)
        real                                 :: duff      ! Duff amount (kg)
        real                                 :: dec_fuel  ! Deciduous litter (kg)
        real                                 :: twig_fuel ! Twig litter (kg)
        ...
        ! Calculate loss from humus layer (t/ha)
        duff_loss = min(C(1, 1)*prem, C(1, 1))

        ! Update N content
        C(1, 1) = max(C(1, 1) - duff_loss, 0.0)
        Nloss = C(1, 2)*prem
        ...
        do i = 2, nc
            ...
            loss = min(C(i, 1)*prem, C(i, 1))
            ! Subtract weight loss
            C(i, 1) = max(C(i, 1) - loss, 0.0)
            ! Subtract N loss
            Nloss = C(i, 2)*prem
            C(i, 2) = max(C(i, 2) - Nloss, 0.0)
            ! Update percent N - though this should be the same?
            C(i, 11) = C(i, 2)/C(i, 1)
        end do

        ! Remove moss
        ! Moss loss (kg)
        m_loss = min(soil%moss_biom*prem, soil%moss_biom)

        ! Remove biomass and recalculate depth
    ...
    !     ! Data dictionary: constants
    !     real, parameter :: QC = 3060   ! Heat of live foliage ignition
    !     real, parameter :: S_0 = 0.05  ! Critical mass flow rate
    ...
    !     real                     :: R_a   ! Critical rate of spread for active crown fire (m/s)
    !     real                     :: I_a   ! Critical intensity for active crowning (kW/m)
    ...
    !         ! Critical rate of spread for active crowning (m/s)
    !         R_a = S_0/canopy_bd

    !         ! Critical intensity for active crowning (kW/m)
    !         I_a = R_a/3.34*HFL*soil%fuel_sum
    !         C_a = soil%I_surf/I_a
    ...
    !             tau_l = sum(tau_b) + 39.4*(canopy_biom/10.0)
    ...
    subroutine prescribed_rosf_from_area(soil, wind_ms, FDI, area_fire, rosf)
        !
        ! Calculates rate of spread from a prescribed burn.
        !
        !  Record of revisions:
        !      Date       Programmer          Description of change
        !      ====       ==========          =====================
        !    08/03/21    A. C. Foster         Original Code
        ...
        real,            intent(in)    :: area_fire ! Prescribed area burnt
        ...
        real :: Uf_max    ! Maximum effective wind speed (m/min)
        real :: Lb        ! Length to breadth ratio of ellipse
        real :: t_fire    ! Fire duration (minutes)
        ...
        rosf = (sqrt(area_fire*HEC_TO_M2/(PI/4.0*Lb))/t_fire)/(1 + exp(-0.012*Uf))


    end subroutine prescribed_rosf_from_area
    ...
    ! Data dictionary: calling arguments
    class(SoilData), intent(inout) :: soil    ! Soil object
    real, dimension(:), allocatable, intent(inout) :: I_r_all      ! Reaction intensity for all fires on site
    real,            intent(in)    :: wind_ms ! Wind speed (m/s)
    ...
    real, dimension(:), allocatable :: I_r_temp ! holder for all I_r's on site
    integer :: lc     ! Looping index

    if(allocated(I_r_all)) then 
        allocate(I_r_temp(size(I_r_all)))
        I_r_temp = I_r_all
        deallocate(I_r_all)
        allocate(I_r_all(size(I_r_temp) + 1))
        I_r_all(1:size(I_r_temp)) = I_r_temp
    else 
        allocate(I_r_all(1))
    end if 
